...
class Sql_creation(object):
	...
	#拆分不同路线
	def split_road_data(self,start_day,end_day,base_len):
		roads=self.__roads
		f=file(u'sql/split_road_data.txt','wb')
		sql="drop table if exists gd_train_data; create table gd_train_data as select * from tianchi_gd.p2_gd_train_data where deal_time>='"+start_day+"' and deal_time<'"+end_day+"';\n"
	...
	def coumun_concat_sql_predict(self,road,type):
		...
		#allroad weather
		sql+=" left outer join  road_"+road+"_allroad_weather_count_"+type+" on recent_day_count_road_"+road+"_"+type+".card_id=road_"+road+"_allroad_weather_count_"+type+".card_id\n"
		#oneroad weather
		sql+=" left outer join  road_"+road+"_oneroad_weather_count_"+type+" on recent_day_count_road_"+road+"_"+type+".card_id=road_"+road+"_oneroad_weather_count_"+type+".card_id\n"
