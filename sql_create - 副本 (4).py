...
class Sql_creation(object):
	...
	def level_sql(self,road,level,r,type):
		...
		d_base=datetime.strptime(str(base_day),'%Y%m%d')
		timestamp=time.mktime(d_base.timetuple())
		f=file(u'sql/'+base_day+'_road_'+road+'_level_2.txt','wb')

		sql=""
		sql+="drop table if exists level_"+level+"_"+type+"; create table level_"+level+"_"+type+" as select \n"
		inner_sql=""
		inner_sql+="(select card_id from gd_train_data_"+road+" where deal_time<'"+base_day+"05' group by card_id) a \n"
		inner_select="a.card_id "
		for i in range(r):
			...
			inner_sql+="left outer join select card_id, cast(count(*) as boolean) as level_"+level+"_"+str(i)+" from gd_train_data_"+road+" where deal_time>='"+dStr2+"' and deal_time<dStr2 group by card_id) table_level_"+level+"_"+str(i)+" on a.card_id=table_level_"+level+"_"+str(i)+".card_id \n"
