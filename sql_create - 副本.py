...
class Sql_creation(object):
	...
	def last_recent_day_count_sql(self,road):
		...
		for x in recent_day:
			...
			sql_str=u"drop table if exists recent_day_count_road_"+road+"_"+str(x)+"; create table recent_day_count_road_"+road+"_"+str(x)+" as select Card_id, count(distinct(datetrunc(to_date(deal_time,'yyyymmddhh'),'DD'))) as day_count_"+str(x)+" from gd_train_data_"+road+" where deal_time>='"+dStr+"' and deal_time<'"+base_day+"05' group by Card_id;"
			f.write(sql_str+'\n') #count(distinct(datetrunc(to_date(deal_time,'yyyymmddhh'),'DD')))
			sql+=sql_str
		#print "abc"
		#print sql
		return sql+'\n'
	...
	def weekday_weekend_count_sql(self,road):
		...
		for i in range(4):
			...
			if i==0:
				inner_sql+="(select card_id,count(distinct(datetrunc(to_date(deal_time,'yyyymmddhh'),'DD'))) as weekday_count_"+index+" from gd_train_data_"+road+" where deal_time>='"+dStr+"' and deal_time<'"+base_day+"05' and weekday(to_date(deal_time,'yyyymmddhh'))<5 group by card_id) weekday_"+index+" join \n"
				inner_sql+="(select card_id,count(distinct(datetrunc(to_date(deal_time,'yyyymmddhh'),'DD'))) as weekend_count_"+index+" from gd_train_data_"+road+" where deal_time>='"+dStr+"' and deal_time<'"+base_day+"05' and weekday(to_date(deal_time,'yyyymmddhh'))>=5 group by card_id) weekend_"+index+" on weekday_1.card_id=weekend_"+index+".card_id join \n"
			elif i==3:
				inner_sql+="(select card_id,count(distinct(datetrunc(to_date(deal_time,'yyyymmddhh'),'DD'))) as weekday_count_"+index+" from gd_train_data_"+road+" where deal_time>='"+dStr+"' and deal_time<'"+base_day+"05' and weekday(to_date(deal_time,'yyyymmddhh'))<5 group by card_id) weekday_"+index+" on weekday_1.card_id=weekday_"+index+".card_id join \n"
				inner_sql+="(select card_id,count(distinct(datetrunc(to_date(deal_time,'yyyymmddhh'),'DD'))) as weekend_count_"+index+" from gd_train_data_"+road+" where deal_time>='"+dStr+"' and deal_time<'"+base_day+"05' and weekday(to_date(deal_time,'yyyymmddhh'))>=5 group by card_id) weekend_"+index+" on weekday_1.card_id=weekend_"+index+".card_id;\n"
			else:
				inner_sql+="(select card_id,count(distinct(datetrunc(to_date(deal_time,'yyyymmddhh'),'DD'))) as weekday_count_"+index+" from gd_train_data_"+road+" where deal_time>='"+dStr+"' and deal_time<'"+base_day+"05' and weekday(to_date(deal_time,'yyyymmddhh'))<5 group by card_id) weekday_"+index+" on weekday_1.card_id=weekday_"+index+".card_id join \n"
				inner_sql+="(select card_id,count(distinct(datetrunc(to_date(deal_time,'yyyymmddhh'),'DD'))) as weekend_count_"+index+" from gd_train_data_"+road+" where deal_time>='"+dStr+"' and deal_time<'"+base_day+"05' and weekday(to_date(deal_time,'yyyymmddhh'))>=5 group by card_id) weekend_"+index+" on weekday_1.card_id=weekend_"+index+".card_id join \n"
	...
	#合并上统计，生成训练表 可扩展
	def concat_recent_day_count_sql(self,road):
		base_day=self.__base_day
		recent_day=self.__recent_day
		f=file(u'sql/'+base_day+'_road_'+road+'_recent_day_count_concat.txt','wb')
		select="gd_train_data_"+road+".card_id"
		for x in recent_day:
			select+=", recent_day_count_road_"+road+"_"+str(x)+".day_count_"+str(x)+" as day_count_"+str(x)


		sql="drop table if exists recent_day_count_road_"+road+"; create table recent_day_count_road_"+road+" as "+"select "+select+" from gd_train_data_"+road;

		for x in recent_day:
			sql_str=" left outer join recent_day_count_road_"+road+"_"+str(x)+" on gd_train_data_"+road+".card_id=recent_day_count_road_"+road+"_"+str(x)+".card_id ";
			sql+=sql_str
	...
	#合并上统计，生成的预测表
	def concat_recent_day_count_sql_predict(self,road):
		base_day=self.__base_day
		recent_day=self.__recent_day
		f=file(u'sql/'+base_day+'_road_'+road+'_recent_day_count_concat.txt','wb')
		select="gd_train_data_"+road+".card_id"
		...
		sql="drop table if exists recent_day_count_road_"+road+"_predict; create table recent_day_count_road_"+road+"_predict as "+"select "+select+" from gd_train_data_"+road;
	...
	#训练集&&正例集=所有用户的标签
	def classify_label_sql(self,road):
		base_day=self.__base_day
		f=file(u'sql/'+base_day+'_road_'+road+'_classify_label.txt','wb')
		sql="drop table if exists label_road_"+road+"; create table label_road_"+road+" as select train_alluser_set_road_"+road+".card_id,regexp_replace(length(positive_set_road_"+road+".card_id) as string,'32','1') as label from train_alluser_set_road_"+road+" left outer join positive_set_road_"+road+" on train_alluser_set_road_"+road+".card_id=positive_set_road_"+road+".card_id;"
		f.write(sql)
		return sql+'\n'

	#将属性列与标签列合并到一个表
	def train_data_sql(self,road,type):
		base_day=self.__base_day
		f=file(u'sql/'+base_day+'_road_'+road+'_train_data.txt','wb')
		sql="drop table if exists "+type+"_data_road_"+road+"; create table "+type+"_data_road_"+road+" as select recent_day_count_road_"+road+".*,label_road_"+road+".label from label_road_"+road+" left outer join recent_day_count_road_"+road+" on label_road_"+road+".card_id=recent_day_count_road_"+road+".card_id;"
		f.write(sql)
		return sql+'\n'
...
def get_data(base_day,road,base_day1,base_day2,base_day3,base_day4,type):
	...
	sql_arr.append(sql_instance.last_recent_day_count_sql(road))
	#合并为一个表
	sql_arr.append(sql_instance.concat_recent_day_count_sql(road))
	#正例集
	sql_arr.append(sql_instance.positive_set_sql(base_day1,base_day2,road))
	#所有用户集
	sql_arr.append(sql_instance.train_alluser_set_sql(base_day3,base_day4,road))
	sql_arr.append(sql_instance.classify_label_sql(road))
	sql_arr.append(sql_instance.train_data_sql(road,type))
...
def main():
	...
	sql_instance.weekday_weekend_count_sql('10')
